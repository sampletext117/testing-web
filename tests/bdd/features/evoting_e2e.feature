Feature: E2E scenario for eVoting with Pytest-BDD
  ...
  Scenario: Successful end-to-end process without external server
    ...
    When a voter is registered with:
      | full_name       | Voter One         |
      | birth_date      | 1990-01-01        |
      | passport_number | VOTER-PASS-123    |
      | issued_by       | Voter Issuer      |
      | issue_date      | 2010-01-01        |
      | country         | Россия            |
    Then the voter_id is returned with status 201

    When a candidate is registered with:
      | full_name           | Candidate One        |
      | birth_date          | 1975-05-05           |
      | passport_number     | CAND-PASS-123        |
    ...
    When an election is created with:
